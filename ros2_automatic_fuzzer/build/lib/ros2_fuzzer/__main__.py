...
from .composition_fuzzer import generate_composition_template
...
def main():
    ...
    for (name, value) in ask_for_components(
        ...
        is_composition = (name, value) in composition.items()
        ...
        if is_service:
            ...
            destination_path = generate_composition_template(
                source=value["source"],
                ros_type_str=value["type"],
                headers_file=value["headers_file"],
            )
            logging.info(f"{name}: created fuzzer for the composition")
            logging.info(f"└── {destination_path}")
    ...
    # for simply checking
    print("Components selected:", [(name, value) for name, value in ask_for_components(
    services=services, topics=topics, actions=actions, composition=composition
)])
