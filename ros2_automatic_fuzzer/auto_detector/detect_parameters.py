...
def detect_parameters(rootDir: str):
    ...
    #composition: dict = yaml_obj["composition"]

    services: dict = yaml_obj.get("services", {})
    topics: dict = yaml_obj.get("topics", {})
    composition: dict = yaml_obj.get("composition", {})
    ...
    for topic_name, topic_value in topics.items():
        if yes_no_question(f"Detect parameters for topic `{topic_name}`?", False):
            # TODO
            pass
    for composition_name, composition_value in composition.items():
        if yes_no_question(f"Detect parameters for composition `{composition_name}`?",False):
            # TODO
            pass
