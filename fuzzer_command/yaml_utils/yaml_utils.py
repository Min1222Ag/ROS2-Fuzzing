...
def read_and_validate_yaml_file(path: str) -> dict:
    ...
    composition_keys= (yaml_obj["composition"] if "composition" in yaml_obj else {}).keys()
    ...
    logging.debug(
        f"{len(composition_keys)} composition detected: {', '.join([f'`{s}`' for s in composition_keys])}"
    )
